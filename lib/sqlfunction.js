...
function createConsultant(name, first_name, price, cost, charges, responsable){
  /** TODO : Make it in one query  */
    queryDatabase("INSERT INTO Consultant(name,first_name,price,cost,charges,responsable) VALUES('"+name+"','"+first_name+"',"+price+","+cost+","+charges+","+responsable+")")
  .then((data)=> { return data})
   return queryDatabase("INSERT INTO Calendar(annee, consultant) values ("+new Date().getFullYear()+", (SELECT consultant.id FROM CONSULTANT where consultant.name='"+name+"' and consultant.first_name='"+first_name+"'))")
...
module.exports = {queryDatabase, createAgent, createConsultant, getListAgents, getListConsultants, registerUser, getUser }