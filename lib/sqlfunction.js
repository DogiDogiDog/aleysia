...
function getCalendarFromConsultants(){
  return queryDatabase("SELECT  janvier, fevrier, mars, avril, mai, juin, juillet, aout, septembre, octobre, novembre, decembre FROM consultant, calendar  where calendar.consultant=consultant.id")
 .then(data=> {return data})
}
...
function getConsultant(id){
  return queryDatabase("SELECT * FROM Consultant WHERE Consultant.id="+id).then((data)=> { return data})
}

module.exports = {getCalendarFromConsultants,updateConsultant, getConsultant,updateConsultantBusinessDay, queryDatabase, createAgent, createConsultant, getListAgents, getListConsultants, registerUser, getUser,getCalendar }