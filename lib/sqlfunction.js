...
function getCalendarFromAgents() {
  return queryDatabase("SELECT calendar.employe_id, janvier, fevrier, mars, avril, mai, juin, juillet, aout, septembre, octobre, novembre, decembre FROM agent, calendar  where calendar.employe_id=agent.id")
    .then(data => {
      return data
    })
}
...
function registerUser(email, password) {
  return queryDatabase("INSERT INTO Credentials(email, psw) VALUES('" + email + "','" + password + "')").then((data) => {
    return data
  })
}
